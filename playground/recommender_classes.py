...
class EDeviceRecommender(BaseRecommender):
    ...
    __y = ['encoded_brand', 'processing_power', 'memory', 'storage_space', 'encoded_storage_type', 'display', 'weight']
    ...
    def fit(self):
        ...
        cls.__ENCODERS['brand'] = labelize(cls.BRANDS)
        cls.__ENCODERS['storage_type'] = labelize(['HDD', 'SSD'])
        cls.data_frame['encoded_brands'] = cls.__ENCODERS['brand'].transform(cls.data_frame['brand'])
    ...
    def predict(self):
        ...
        if brand_encoder := cls.__ENCODERS.get('brand', None):
        ...
        if storage_type_encoder := cls.__ENCODER.get('storage_type', None):
