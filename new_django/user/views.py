...
class HistoryView(APIView):
    ...
    def get(self, request, index, *args, **kwargs):
        ...
        cartOrders = CartOrder.objects.filter(user__username='emilyjim1').order_by('-date').prefetch_related(Prefetch('product', queryset=products))
        cartOrders = paginate_queryset(cartorders, request, 20, CartOrderSerializer)
        
        singleProductOrders = SingleProductOrder.objects.filter(user__username='emilyjim1').order_by('-date').select_related('product').only('id', 'name')
